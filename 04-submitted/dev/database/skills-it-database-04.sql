...
-- Script date 2020. 03. 07. 12:15:18
...
DROP DATABASE IF EXISTS skills_it_04;

CREATE DATABASE skills_it_04
	CHARACTER SET utf8
...
--
ALTER TABLE flats 
  ADD CONSTRAINT FK_flats_building_id FOREIGN KEY (building_id)
    REFERENCES buildings(id) ON DELETE CASCADE ON UPDATE CASCADE;

DELIMITER $$

--
-- Create procedure `flat_update`
--
CREATE DEFINER = 'root'@'localhost'
PROCEDURE flat_update(
  IN _id INT(11)
   ,IN  _building_id int(11)
  ,IN  _ref          int(11)
  ,IN  _floor        int(11)
  ,IN  _rooms        decimal(3,1)
  ,IN  _orientation  VARCHAR(2)
  ,IN  _net_area     decimal(5,1)
  ,IN  _balcony_area decimal(4,1) 
  ,IN  _price        decimal(5,2) 
  ,IN  _sold          int(11)
  )
BEGIN
UPDATE flats
SET building_id = _building_id,
    ref = _ref,
    floor = _floor,
    rooms = _rooms,
    orientation = _orientation,
    net_area = _net_area,
    balcony_area = _balcony_area,
    price = _price,
    sold = _sold
WHERE id = _id;
...
SELECT
  ...
  `buildings`.`number_of_floors` AS `number_of_floors`,
  `buildings`.`address` AS `address`,
  COUNT(`flats`.`id`) AS `count_of_flats`
FROM (`buildings`
  JOIN `flats`
    ON ((`buildings`.`id` = `flats`.`building_id`)))
...
GRANT Select (Insert_priv), Select (References_priv), Select (Select_priv), Select (Grant_priv), Select (User), Select (Repl_client_priv), Select (File_priv), Select (Host), Select (Repl_slave_priv), Select (Process_priv), Select (Execute_priv), Select (Shutdown_priv), Select (Lock_tables_priv), Select (Reload_priv), Select (Create_tmp_table_priv), Select (Drop_priv), Select (Super_priv), Select (Create_priv), Select (Show_db_priv), Select (Delete_priv), Select (Alter_priv), Select (Update_priv), Select (Index_priv) ON  mysql.user TO 'pma'@'localhost';
GRANT Select (Db), Select (Host), Select (Column_priv), Select (Table_priv), Select (Table_name), Select (User) ON  mysql.tables_priv TO 'pma'@'localhost';
...
CREATE USER 'skill17'@'localhost' IDENTIFIED WITH mysql_native_password;
GRANT Usage ON *.* TO 'skill17'@'localhost' 
WITH GRANT OPTION;
SET PASSWORD FOR 'skill17'@'localhost' = '*3D727D2497BFA55067A689E13DE6CF5EEA1B400C';
GRANT ALL PRIVILEGES ON *.* TO 'skill17'@'localhost';
